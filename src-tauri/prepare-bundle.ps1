...
# Cargo.toml's default-run) plus four more binaries that need to end up somewhere OTHER than next
# to the main exe for an installed copy to actually work:
#
...
#     lowarc_runtime.exe is what "Export" actually copies into every exported project (see
#     export/runtime_source.rs) — it's a genuine binary target of this same workspace, built the
#     exact same way as everything else here, no separate repo or build step involved anymore.
#
# All three destinations feed tauri.conf.json's bundle.resources, which is what actually gets these

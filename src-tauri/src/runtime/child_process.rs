...
// parsing a {"log":{"severity":..}} notification's severity string, all previously copy-pasted
// across those three call sites independently. process_module's own copy of command resolution
// was missing the Windows .exe fallback the other two already had — a real bug, not just
// duplication: a module whose manifest omitted the extension would fail to launch on Windows,
...
/// process spamming stderr can't flood the log with a single giant line. Was two independent
/// magic `300`s before this had a name; kept as one shared constant even though the two callers
/// truncate slightly different things (one whole accumulated stderr buffer for an error message,
/// one line at a time as it streams in) — same budget, same reasoning, worth staying in sync.
